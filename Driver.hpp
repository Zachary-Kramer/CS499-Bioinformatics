...
inline void Run(const std::string& filePath,
	...
	const Index numKmers = maxMemory / substringLength;
    ...
    for (i = 0; i < sequences.size(); ++i) {
		...
		for (Index j = 0; j <= numIters; j++) {
			const auto substrings = GetSubstrings(j * numKmers, substringLength, numKmers, sequences.at(i).DNA);
